...
<head>
  ...
  <title>จัดการสมาชิก | CE Mooc</title>
                            ...
                            <div class="card-body">
                                <div class="px-5">
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-xl-10">
                                            <a href="../users/profile.php?id=1">kasidit sonrod</a>
                                        </div>
                                    </div>
                                    <div class="row mb-3">
                                        <p class="col-xl-1 text-muted">อีเมล :</p>
                                        <div class="col-xl-10">
                                            <p>kasidit.sor@gmail.com</p>
                                        </div>
                                    </div>
                                    <div class="row mb-3">
...
<script>
    $(function() {
        $.ajax({
            type: "GET",
            url: "../../service/users/profile.php"
        }).done(function(data) {
            let tableData = []
            data.response.forEach(function (item, index){
                tableData.push([    
                    `<a href="../orders/info.php?o_id=${item.o_id}" class="btn btn-outline-primary p-1 "> ${item.o_id} </button>`,
                    `<span class="badge badge-success"> ${item.status ? 'ผ่านแล้ว': 'ยังไม่ผ่าน'} </span>`,  
                 
                    `<span class="text-muted"> ${item.cat_name} </span> `,
        ...
        function initDataTables(tableData) {
            $('#logs').DataTable( {
                ...
                columns: [
                    ...
                    { title: "รายชื่อวิชา ", className: "align-middle"},
                    { title: "วันลงทะเบียน", className: "align-middle"},
                    { title: "หมดเวลา", className: "align-middle"},
